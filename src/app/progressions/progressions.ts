import { numToString, scalepatterns, generateScales, noteNames, chordpatterns, generateChords } from '../data';
...
export class ProgressionQuestionGenerator {
  ...
  private enableChords = {"": true, "7": true, "maj7": false, "m": true, "m7": true, "mmaj7": false, "dim": false, "dim7": false,};
  ...
  public generateScaleNames() {
    ...
    for(let root = 21; root < 33; root++) {
      ...
      if (
        ...
        for(let pat of Object.keys(scalepatterns).filter(pat => this.enableScaleModes[pat])) {
  ...
  public generateChords() {
    this.scale = this.enabledScaleNames[Math.floor(Math.random() * this.enabledScaleNames.length)];
    ...
    let rootChords = enabledChordNames.filter(chord => chord.split(" ")[0] == this.scale.split(" ")[0]);
    this.progression.push(rootChords[Math.floor(Math.random() * rootChords.length)])
    let notLooping = true;
    while(notLooping) {
      while (this.progression.length < 4) {
        ...
        for (let cn of enabledChordNames) {
          ...
          if (this.progression.filter(chord => chord.split(" ")[0] == cn.split(" ")[0]).length == 0) {
        ...
        this.progression.push(potenantialNextChords[Math.floor(Math.random() * potenantialNextChords.length)])
      }
      console.log("aaa", this.chords[this.progression[this.progression.length -1]], this.chords[this.progression[0]])
      if (chordIntersection(this.chords[this.progression[this.progression.length -1]], this.chords[this.progression[0]]).length > 1) {
        notLooping = false;
      } else {
